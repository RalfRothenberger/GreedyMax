...
int Greedy(PNGraph G, FILE* F, TInt Budget) {
	std::set<int> solution, rest;
	//diff counts the difference in solution size each node (not in the solution) contributes
	...
	for (int k = 0; k < Budget; ++k) {
		...
		//by adding them, their contribution will be gone, reducing their increase of the target function by one
...
int main(int argc, char* argv[]) {
	...
	Try{
		...
		if (P > 0) {
			appendix += "-P-";
			appendix += std::to_string(int(P));
		}
		else {
			appendix += "-B-";
			appendix += std::to_string(int(Budget));
		}
